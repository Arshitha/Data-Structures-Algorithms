...
/******* From here down is the content of the LinkedList.cpp file: ***********************/
 ...
 void LinkedList<T>::InsertToTail(T data)
 {
    ...
    if (head == nullptr)                       //checks if the linkedList is empty
        head = newNode;                         //if true assigns the newNode as the head
    
    else 
    {
        Node<T>* currNode = head;
        while(currNode->next != nullptr)
        {                                //checking if currentNode's next pointer is a null or not
            currNode = currNode->next;   //assigning the currentNode's next pointer as currentNode
        }
        
        currNode->next = newNode;         //inserting a new node to tail
    }
}
